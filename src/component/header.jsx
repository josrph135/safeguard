  ...
  return (
    <header className='bg-black w-full'>
        ...
        <nav className='hidden lg:block'>
            {link.map((link, i) => 
                <ul className='flex flex-row divide-x-2 divide-white md:text-base  font-semibold ' key={i}>
                    {link.title.map((link,i) => 
                    <li className='px-4' key={i}><a className='hover:border-b-2 duration-300 border-b-2 border-black hover:text-white/90 text-white/70 hover:border-primary-0 pb-1' href={`#${link}`}>{link}</a></li>)}
                </ul>
            )}
        </nav>
        ...
        <section className={`fixed ${open ? null : "hidden"} bg-black z-50 top-0 left-0 right-0`}>
            ...
            <ul className='flex flex-col pt-9 text-base text-white  justify-center'>
